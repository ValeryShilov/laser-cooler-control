...
class TopologyLayoutEngine:
    ...
    def _cycle_rect_positions(self, n):
        ...
        Координаты подобраны по эталонному SVG.
        """
        corners = [
            (0.12, 0.75),    # низ-лево  (Компрессор)
            (0.12, 0.15),    # верх-лево (Конденсатор)
            (0.32, -0.12),   # верх-центр (Дроссель) — выше padding для верности
            (0.50, 0.45),    # центр      (Бак / Испаритель)
        ]
        if n <= 4:
            return corners[:n]
